;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;;
;;;; 04_bulls-eye.lisp - příklad k přednášce 4. Dědičnost
;;;;
...
Třída pracuje s vlastností items, proto by nebylo dobré, aby ji nastavoval
uživatel. 
|#
...
(defmethod initialize-instance ((be bulls-eye) &key)
  (call-next-method)
  (set-items be (make-be-items 0 0 1 1)))
...
(defmethod set-circle-count ((be bulls-eye) count)
  ...
  (set-items be (make-be-items (x (center be))
                               (y (center be))
                               (radius be)
                               count)))
