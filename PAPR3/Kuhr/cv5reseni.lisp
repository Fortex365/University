...
;; trida hideable-picture (varianta 1)

(defclass hideable-picture (picture)
  ((hide-items :initform  '())))

(defmethod hide-items ((hp hideable-picture))
  (copy-list (slot-value hp 'hide-items)))

(defmethod set-hide-items ((hp hideable-picture) value)
    (when (not (= (length value) (length (items hp))))
      (error "Length of hide-items should be the same as the length of items. "))
    (setf (slot-value hp 'hide-items) (copy-list value))
    hp)

(defmethod set-items ((hp hideable-picture) value)
  (call-next-method)
  (when (not (= (length value) (length (hide-items hp))))
    (set-hide-items hp (make-list (length value) :initial-element t)))
  hp)

(defmethod draw ((hp hideable-picture) mg-window)
  (let ((len (length (items hp))))
    (dotimes (i len)
      (when (nth (- len i 1) (hide-items hp))
        (draw (nth (- len i 1) (items hp)) mg-window))))
  hp)


;; trida hideable-picture (varianta 2)

(defclass hideable-picture2 (abstract-picture)
  ((hide-items :initform  '())
   (all-items :initform '())))

(defmethod hide-items ((hp hideable-picture2))
  (copy-list (slot-value hp 'hide-items)))

(defmethod set-hide-items ((hp hideable-picture2) value)
    (when (not (= (length value) (length (all-items hp))))
      (error "Length of hide-items should be the same as the length of items. "))
    (setf (slot-value hp 'hide-items) (copy-list value))
    hp)

(defmethod all-items ((hp hideable-picture2)) 
  (copy-list (slot-value hp 'all-items)))

(defmethod set-all-items ((hp hideable-picture2) value)
  (do-check-items hp value)
  (setf (slot-value hp 'all-items) (copy-list value))
  (when (not (= (length value) (length (hide-items hp))))
    (set-hide-items hp (make-list (length value) :initial-element t)))
  hp)

(defmethod items ((hp hideable-picture2))
  (let ((items '())
        (len (length (all-items hp))))
    (dotimes (i len)
      (when (nth (- len i 1) (hide-items hp))
        (setf items (cons (nth (- len i 1) (all-items hp)) items))))
   items))

(defmethod send-to-items ((hp hideable-picture2) 
			  message
			  &rest arguments)
  (dolist (item (all-items hp))
    (apply message item arguments))
  hp)
...
(set-hide-items hp '(t t t))
(set-hide-items hp '(t nil))
(redraw w)
(set-hide-items hp '(nil t))
(redraw w)
(move hp 50 0)
(set-hide-items hp '(t t))
(redraw w)

(set-items hp (list tri sq (make-instance 'point)))
(redraw w)


(setf hp (make-instance 'hideable-picture2))
(set-all-items hp (list tri sq))
(set-shape w hp)
(redraw w)
...
(set-all-items hp (list tri sq (make-instance 'point)))
(set-items hp (list tri sq (make-instance 'point)))
(redraw w)
|#
